...
def generate_launch_description():
    ...
    output_string_topic_arg = DeclareLaunchArgument(
        name='output_string_topic',
        default_value=config['output_string_topic'],
        description='Name of the topic to publish the string with the detected markers',
    )
    ...
    aruco_node = Node(
        ...
        parameters=[{
            ...
            "output_string_topic": LaunchConfiguration('output_string_topic'),
        }],
        output='screen',
        emulate_tty=True
    )
